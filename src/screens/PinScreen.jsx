...
export default function PinScreen({ onUnlock, pinHash, C }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: "flex", gap: 16,
        animation: error ? `shake 400ms ${springs.snap}` : "none",
      }}>
        <style>{`
          @keyframes shake {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 72px)", gap: 14 }}>
        {keys.map((key, i) => (
          <button key={i} onClick={() => key === "⌫" ? handleDelete() : key ? handleKey(key) : null}
            disabled={!key}
            style={{
              width: 72, height: 72, borderRadius: "50%", border: "none", cursor: key ? "pointer" : "default",
      ...
      {error && (
        <div style={{ fontSize: 13, color: C.expense, fontWeight: 500 }}>Incorrect PIN. Try again.</div>
      )}
    </div>
  );
}