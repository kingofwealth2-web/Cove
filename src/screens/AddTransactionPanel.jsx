...
export default function AddTransactionPanel({ onClose, onSave, categories, setCategories, user, C, fxRates = {}, templates = [], onSaveTemplates }) {
  ...
  const [newCatIcon, setNewCatIcon] = useState("\u{1F4B0}");
      ...
      fetch("https://open.er-api.com/v6/latest/" + user.currency)
        ...
        .then(d => {
          if (d.rates?.[selectedCurrency]) {
            setCustomRate(String(parseFloat((1 / d.rates[selectedCurrency]).toFixed(6))));
          } else {
            setCustomRate("");
          }
        })
  ...
  const handleSave = () => {
    ...
    if (splitMode && splitCatA && splitCatB) {
      ...
      onSave({
        ...
        amount: amtA, type,
        note: note ? (note + " (split " + splitPct + "%)") : ("Split with " + (catBObj?.name || "") + " (" + splitPct + "%)"),
      ...
      onSave({
        ...
        amount: amtB, type,
        note: note ? (note + " (split " + (100 - splitPct) + "%)") : ("Split with " + (catAObj?.name || "") + " (" + (100 - splitPct) + "%)"),
        date, isRecurring: false,
        originalCurrency: isForeign ? selectedCurrency : null,
  ...
  const ICONS = ["\u{1F4B0}", "\u{1F4BC}", "\u{1F4BB}", "\u{1F3EA}", "\u{1F381}", "\u{1F4E6}", "\u{1F331}", "\u{1F91D}", "\u{1F3E0}", "\u{1F697}"];
  ...
  return (
    <SlidePanel onClose={onClose} C={C} title="Log Transaction">
              ...
              {templates.map(tpl => (
                <div key={tpl.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <button onClick={() => applyTemplate(tpl)} style={{
                    ...
                    <span>{tpl.icon}</span>
                    {tpl.label}{tpl.amount ? " - " + user.currency + " " + tpl.amount : ""}
                  </button>
      ...
      {/* Amount */}
      <div style={{ padding: isMobile ? "20px 28px 16px" : "20px 28px", borderBottom: "1px solid " + C.border }}>
        {!isMobile && <Label C={C} style={{ marginBottom: 8 }}>Amount</Label>}
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: C.surfaceAlt, borderRadius: 14, padding: isMobile ? "14px 18px" : "12px 16px", border: "1px solid " + C.border }}>
          <span style={{ fontSize: isMobile ? 17 : 15, fontWeight: 600, color: C.textMuted }}>{selectedCurrency}</span>
          <input
            autoFocus
            inputMode="decimal"
            ...
            style={{ flex: 1, background: "none", border: "none", outline: "none", fontSize: isMobile ? 36 : 28, fontFamily: "'DM Serif Display', serif", color: C.text, letterSpacing: "-1px", width: "100%" }}
          />
        </div>
        {isForeign && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10, padding: "10px 14px", background: C.accentSoft, borderRadius: 12, border: "1px solid " + C.accent + "22" }}>
            ...
            {parsedAmount > 0 && rate > 0 && (
              <span style={{ fontSize: 13, fontWeight: 700, color: C.accent, marginLeft: 4 }}>
                {"=> " + user.currency + " " + baseAmount.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </span>
            )}
          </div>
        )}
        ...
        {splitMode ? (
            ...
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                <span style={{ fontSize: 11, color: C.textMuted }}>
                  {categories.find(c => c.id === splitCatA)?.icon} {categories.find(c => c.id === splitCatA)?.name || "Category 1"}
                </span>
                <span style={{ fontSize: 11, color: C.textMuted }}>
                  {categories.find(c => c.id === splitCatB)?.name || "Category 2"} {categories.find(c => c.id === splitCatB)?.icon}
                </span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            ...
            {type === "income" && catId && !showNewCat && (() => {
              const sel = categories.find(c => c.id === catId);
              return sel ? null : (
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === "Enter" && newCatName.trim()) {
                      ...
                      setShowNewCat(false);
                      setNewCatName("");
                      setNewCatIcon("\u{1F4B0}");
                    }
                    if (e.key === "Escape") { setShowNewCat(false); setNewCatName(""); }
                  }}
                  style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 10, padding: "9px 12px", fontSize: 14, color: C.text, outline: "none", width: "100%" }}
                />
      ...
      <div style={{ padding: "14px 28px", borderBottom: "1px solid " + C.border, display: "flex", flexDirection: "column", gap: 10 }}>
        <input value={note} onChange={e => setNote(e.target.value)}
          placeholder={splitMode ? "Add a note (applies to both splits)..." : "Add a note..."}
      ...
      <div style={{ padding: "8px 28px 28px" }}>
        {type === "income" && !catId && !splitMode && parsedAmount > 0 && (
          <div style={{ fontSize: 12, color: "#FF9F0A", padding: "8px 12px", background: "rgba(255,159,10,0.1)", borderRadius: 10, border: "1px solid rgba(255,159,10,0.25)", marginBottom: 8, textAlign: "center" }}>
            No income category selected - tap a category above first
          </div>
        )}
